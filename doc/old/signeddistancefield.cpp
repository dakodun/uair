...
std::pair<std::vector<unsigned char>, glm::ivec2> SignedDistanceField::GenerateFromTexture(Texture* texture, const unsigned int& layer) {
	...
	auto lambdaCheckAdjacent = [&lambdaGetIndex, &binaryMap, &textureWidth, &textureHeight](const size_t& x, const size_t& y)->bool {
		...
		if (    (x > 0                 && binaryMap.at(lambdaGetIndex(x - 1, y)) != binaryMap.at(lambdaGetIndex(x, y))) || // check left pixel
	...
	// forward pass:
	// perform first propagation pass (left, bottom-left, bottom, bottom-right)
	for (size_t y = 0u; y < textureHeight; ++y) { // for all rows...
		for (size_t x = 0u; x < textureWidth; ++x) { // for all columns...
			lambdaPropagate(x, y, -1, -1, distDiag); // left
			 lambdaPropagate(x, y,  0, -1, distAdj); // bottom-left
			lambdaPropagate(x, y,  1, -1, distDiag); // bottom
			 lambdaPropagate(x, y, -1,  0, distAdj); // bottom-right
		}
	}
	...
	std::vector<float> distanceFieldScaled;
	unsigned int scale = 4u;
	unsigned int scaledWidth = textureWidth / scale;
	unsigned int scaledHeight = textureHeight / scale;
		...
		for (size_t x = 0u; x < textureWidth; x += scale) { // for all columns...
			...
			for (size_t dy = 0u; dy < scale; ++dy) {
				for (size_t dx = 0u; dx < scale; ++dx) {
					accum += distanceField.at(lambdaGetIndex(x + dx, y + dy));
				}
			}
			
			accum /= (scale * scale);
            distanceFieldScaled.push_back(accum);
		}
	}
	...
	for (size_t y = 0u; y < scaledHeight; ++y) { // for all rows...
		for (size_t x = 0u; x < scaledWidth; ++x) { // for all columns...
			float thisDistance = distanceFieldScaled.at(x + (y * scaledWidth));
            float clampDist = std::max(-normFactor, std::min(thisDistance, normFactor));
            float scaledDist = clampDist / normFactor;
            unsigned char finalDistance = (((scaledDist + 1.0f) / 2.0f) * 255.0f) + 0.5f;
