	...
	public :
		...
		class StoreEntry {
			public :
				...
				StoreEntry(const Ps&... params) : mData(params...) {
		...
		template <typename ...Ps>
		std::pair<unsigned int, unsigned int> Add(const Ps&... params) {
			if (!mFreeIndices.empty()) { // if there exists a previously used index that is now free...
				...
				if (freeIndex < mStore.size() && !mStore.at(freeIndex).mActive) {
					...
					StoreEntry newEntry(params...); newEntry.mCounter = counter;
				...
				mStore.emplace_back(params...); // add a new entry to the store
		...
		void Remove(const unsigned int& index, const unsigned int& counter) {
			...
			if (index < mStore.size() && mStore.at(index).mActive && mStore.at(index).mCounter == counter) {
				...
				mFreeIndices.push(index); // add it's index to the sorted queue of now free indices
			}
			else {
...
class Manager {
	public :
		...
		class Handle {
			public :
				Handle(const unsigned int& typeID, const unsigned int& index, const unsigned int& counter) : mTypeID(typeID), 
		...
		// add a new resource of the specified type to the store and return a custom handle to it
		template <typename T, typename ...Ps>
		Handle Add(const Ps&... params) {
			if (!std::is_base_of<B, T>::value) { // if resource type to register does not inherit from base class...
			...
			try {
				indexCounterPair = storePtr->Add(params...); // add a new entry to the store and save the index and counter value returned
			} catch (std::exception& e) {
				throw;
			}
			
			return Handle(typeID, indexCounterPair.first, indexCounterPair.second); // return a handle to the newly added resource
		}
