...
class EXPORTDLL InputManager {
	...
	public :
		...
		bool DeviceExists(const int& deviceID) const; // returns true if the input device with the specified id exists
		const InputDevice& GetDevice(const int& deviceID) const; // returns the input device with the associated id (or the default device if id is invalid)
